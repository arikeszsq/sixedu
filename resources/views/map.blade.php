...
<div style="position:relative;">
    <div style="height:53px;">
        <div class="search">
            <div class="search_c">
                <input id="btn_content" type="text" class="search_t"
...
<script type="text/javascript">
    ...
    const points = parent.getCoordinate(); // 回填坐标点
    var container = document.getElementById("container");
    const map = new qq.maps.Map(container, {
        center: new qq.maps.LatLng(points[0], points[1]),
        zoom: 10
    });
    ...
    //创建一个Marker
    const markerInit = new qq.maps.Marker({
        //设置Marker的位置坐标
        position: new qq.maps.LatLng(points[0], points[1]),
        //设置显示Marker的地图
        map: map
    });
    ...
    qq.maps.event.addListener(map, "click", function (e) {
        ...
        $.getJSON(url3, function (result) {
            if (result.result != undefined) {
                parent.setCoordinateArea(result.result.address);
                document.getElementById("btn_content").value = result.result.address;
            } else {
                parent.setCoordinateArea("");
                document.getElementById("btn_content").value = "";
            }
        })
    });
    ...
    function setFlagClicked(arr, index) {
        each(markerArray, function (n, ele) {
            if (n == index) {
                ...
                url3 = encodeURI("https://apis.map.qq.com/ws/geocoder/v1/?location=" + latLng.getLat().toFixed(6) + "," + latLng.getLng().toFixed(6) + "&key=ZWEBZ-R7N3U-BJSVH-4TCR3-66MDQ-S3FDJ&output=jsonp&&callback=?");
                $.getJSON(url3, function (result) {
                    if (result.result != undefined) {
                        parent.setCoordinateArea(result.result.address);
                        document.getElementById("btn_content").value = result.result.address;
